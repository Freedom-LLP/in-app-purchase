...
function validateProduct(receipt, tokenMap, cb) {
    ...
    function _onProductValidate(error, res, body) {
        ...
        // A 5xx is Google failing, not a verdict on the receipt. Reporting it as FAILURE would
        // have the caller void a purchase that was very likely paid for, so it is kept distinct.
        if (res.statusCode >= 500) {
            verbose.log(NAME, 'Product validation failed, server error:', body, res.statusCode);
            return cb(new Error('Status:' + res.statusCode), createErrorData(receipt, {
...
function createErrorData(receipt, obj) {
    ...
    // Tag the store on failures too. A caller holding only a rejected promise otherwise cannot
    // tell which store rejected, which matters when one app validates against several.
    obj.service = constants.SERVICES.GOOGLE;
    return obj;
}
