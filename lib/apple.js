...
module.exports.validatePurchase = function (secret, receipt, cb) {
    ...
    var tryProd = function (next) {
        ...
        send(prodPath, content, function (error, res, data) {
            ...
            if (error || (data && data.status === 21005)) {
                ...
                // instead of voiding a purchase that may be perfectly good.
                status = data ? data.status : constants.VALIDATION.SERVER_ISSUE;
                validatedData = {
                    sandbox: false,
                    status: status,
    ...
    var trySandbox = function (next) {
        ...
        send(sandboxPath, content, function (error, res, data) {
            ...
            if (error || (data && data.status === 21005)) {
                // Same reasoning as the production path above.
                status = data ? data.status : constants.VALIDATION.SERVER_ISSUE;
                validatedData = {
                    sandbox: true,
                    status: status,
