...
module.exports.validatePurchase = function (secret, receipt, cb) {
    ...
    var tryProd = function (next) {
        ...
        send(prodPath, content, function (error, res, data) {
            ...
            // request error
            if (error || (data && data.status === 21005)) {
                // No verdict on the receipt: either we never reached Apple, or Apple says its
                // own receipt server is down (21005). Report SERVER_ISSUE so the caller retries
                // instead of voiding a purchase that may be perfectly good. SERVER_ISSUE whichever
                // way we got here — passing 21005 through as the status made the caller treat it
                // as just another Apple rejection, which voids a purchase the player paid for.
                status = constants.VALIDATION.SERVER_ISSUE;
                validatedData = {
                    sandbox: false,
                    status: status,
                    message: (data && errorMap[data.status]) || 'Unknown',
                    service: constants.SERVICES.APPLE
                };
                applyResponseData(validatedData, data);
                verbose.log('<Apple>', prodPath, 'failed:', error, validatedData);
                // On 21005 Apple answered over a healthy connection, so there is no Error to
                // decorate — `error` is null and assigning to it throws, which in a host that
                // exits on uncaughtException is a process kill per receipt.
                var prodError = error || new Error(validatedData.message);
                prodError.validatedData = validatedData;
                return next(prodError);
            }
    ...
    var trySandbox = function (next) {
        ...
        send(sandboxPath, content, function (error, res, data) {
            ...
            if (error || (data && data.status === 21005)) {
                // Same reasoning as the production path above, including the null `error`.
                status = constants.VALIDATION.SERVER_ISSUE;
                validatedData = {
                    sandbox: true,
                    status: status,
                ...
                var sandboxError = error || new Error(validatedData.message);
                sandboxError.validatedData = validatedData;
                return next(sandboxError);
            }
